...
class Bmv2_grpc_tables_api():
    ...
    """
    def table_cam_read_entry(self, table_name, keys):
        if table_name not in self.table_cache: return (False, '', [])
        table = self.table_cache[table_name]
        for entry in table:
            if entry["match_field"] == keys:
                return (True, entry["action_params"], entry["action_params"])
        return (False, '', [])
...
class Tables_populator():
    ...
    def load_ifaces(self, ifaces):
        """
        TODO
        E.g., self.api.table_cam_add_entry(TABLE_NAME, match_fields={...}, action_name='...', action_params={...}) {'p.ip.dstAddr': iface.ip}
        """
        for iface in ifaces:
            self.api.table_cam_add_entry(LOCAL_IP_TABLE_NAME, match_fields={'p.ip.dstAddr': iface.ip}, action_name='MyIngress.local_hit', action_params={})
    ...
    def load_arp_cache(self, arp_cache):
        """
        TODO
        E.g., self.api.table_cam_add_entry(TABLE_NAME, match_fields={...}, action_name='...', action_params={...}) {"next_hop_ipv4": a[0]}
        """
        for a in arp_cache:
            self.api.table_cam_add_entry(ARP_CACHE_TABLE_NAME, match_fields={"next_hop_ipv4": a[0]}, action_name='MyIngress.arp_respond', action_params={"result": a[1]})